...
class Content{
    ...
    public static function BreadcrumbNavigation($archive, $WebUrl){
        ...
        if (@in_array("sreenshot",$options->featuresetup) && $archive->is("post")){
            $screenshotStyle='i.fontello.fontello-weibo:after {
    padding: 0 5px 0 5px;
    color: #ccc;
    content: "/\00a0";
    }';
        ...
        while ($obj->next()){
            /*if (self::isImageCategory($obj->categories)){
                ...
                if ($styleThumb == 'SMALL'){//文章页面选择小版式
                    ...
                    if (in_array('smallStyleThumb', $options->indexsetup)){
                        $parameterArray['isBig'] = false;
                    }else{
                        $parameterArray['isBig'] = true;
                    }
...
EOF;
                    ...
                    $backgroundImageHtml = Utils::returnDivLazyLoadHtml($parameterArray['imgSrc'],250,0);
                    $html .= <<<EOF
...
EOF;
            //摘要部分
            $html .= <<<EOF
<p class="summary">{$parameterArray['summary']}</p>
EOF;
            //页脚部分，显示评论数、作者等信息
            $html .= <<<EOF
...
EOF;
     ...
     * @return mixed
     */
    public static function postCommentContent($content,$isLogin){
        //解析表情
        ...
        if ( strpos( $content, '[secret]')!== false) {//提高效率，避免每篇文章都要解析
            ...
            if (!$isLogin){
                return '<div class="hideContent">该评论仅登录用户可见</div>';
            }
        }
        return $content;
    }
    ...
    public static function postContent($obj,$status){
        ...
        if ($isImagePost){//照片文章
            ...
            if($status || $result) {
                ...
                $content = preg_replace("/\[hide\](.*?)\[\/hide\]/sm",'<div class="hideContent">'._mt("此处内容需要评论回复后方可阅读。").'</div>',$content);
            }

            //文章中折叠框功能
            if (strpos($content, '[collapse') != false){
                $pattern = self::get_shortcode_regex(array('collapse'));
                $content = Utils::handle_preg_replace_callback("/$pattern/",array('Content','collapseParseCallback'),
                    $content);
            }
            ...
            if ( strpos( $content, '[post')!== false) {//提高效率，避免每篇文章都要解析
                $pattern = self::get_shortcode_regex(array('post'));
                $content = Utils::handle_preg_replace_callback("/$pattern/",array('Content','quoteOtherPostCallback'), $content);
            }
            ...
            if ( strpos( $content, '[login')!== false) {//提高效率，避免每篇文章都要解析
                ...
                $content = Utils::handle_preg_replace_callback("/$pattern/",function ($matches) use ($isLogin) {
                    ...
                    if($isLogin){
                        ...
                        return '<div class="hideContent">'._mt("该部分内容仅登录用户可见").'</div>';
                    }
                }, $content);
            }
...
var registCommentEvent = function() {
    ...
    public static function slectNavbarHeader(){
        ...
        switch ($options->themetype){
            ...
            case 12:
                $html .= '<div class="navbar-header bg-success">';break;
            case 13:
                $html .= '<div class="navbar-header bg-danger dker bg-gd">';break;
            default:
                $html .= '<div class="navbar-header bg-danger dker bg-gd">';break;
        }
        return $html;
    ...
    public static function selectNavbarCollapse(){
        ...
        switch ($options->themetype){
            ...
            case 13:
                $html .= '<div class="collapse pos-rlt navbar-collapse box-shadow bg-danger dker bg-gd">';break;
            default:
                $html .= '<div class="collapse pos-rlt navbar-collapse box-shadow bg-danger dker bg-gd">';break;
        }
